...
df[["installment","loan_amnt","mo_sin_old_rev_tl_op","earliest_cr_line","total_acc","open_acc", 
"pub_rec_bankruptcies", "pub_rec"]].isnull().any()
...
####### QUESTION #######
1. Grouping based features should be done as train_df and test_df separtely or
    by concatenating both in on edata DataFrame
2. Add

How to add diversity in the input data:
...
Feature engineering in meta modelling
    - pair-wise difference b/w meta feature to reduce correlation
    - row-wise stats like averages or std 
    - standard feature selection

Use 1 meta models for 7.5 base models.



Remove outlier and fill with nan values, now run factor-matrixization 
on this dataset, to fill those value, advantage is that, there will be 
smoothness in the features, then run some classifiers on top of that.
...
Each method, will bring new infomation to the ensembles

Cat feature from decision tree:
    Tree based model:
        - tree_model.apply()
    XGBoost:
...
###################################################################
###################################################################
df contains 3 columns: ['user_id','page_id','ad_price']
