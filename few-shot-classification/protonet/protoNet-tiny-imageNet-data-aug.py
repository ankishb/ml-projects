...
def compute_loss(Flag, model, support, query, labels, num_classes, num_support, num_queries, emb_dim):
    num_classes, num_support, num_queries, emb_dim = num_classes, num_support, num_queries, emb_dim
    # num_classes, num_support, num_queries = 5, 1, 1
...
#     print("checking dtypes: ", d_support.dtype, d_query.dtype, y.dtype)
    ...
    print("*************",Flag)
    if Flag == 1:
#         model.forward(d_support)
#         variables = model.variables#_eager_var_store._store._vars
#         saver = tfe.Saver(variables)
#         checkpoint_prefix = os.path.join('./model_dir_nway10', "ckpt")
...
def compute_gradients(Flag, model, support, query, labels, 
            num_classes, num_support, num_queries, emb_dim):
    with tf.GradientTape() as tape:
#         loss = compute_loss(model, x)
        loss = compute_loss(Flag, model, support, query, labels, 
            num_classes, num_support, num_queries, emb_dim)
...
checkpoint_dir = './model_dir_nway10'
...
lr_rate = tfe.Variable(1e-4, trainable=False)
...
#########################
#############
# tfe.Saver((model.variables + optimizer.variables())).restore(tf.train.latest_checkpoint(checkpoint_dir))
...
#     )
    ...
    for epi in range(n_episodes):
        ...
        gradients, loss = compute_gradients(Flag, model, support, query, labels, 
...
#         print(decayed_learning_rate)
    print("epoch: {}, lr_rate: {}, tr_loss: {}, val_loss: {}, val_acc: {}".format(
        epoch, np.round(decayed_learning_rate,4), 
...
test_acc = []
for i in range(100):
    support, query, labels = get_batch(test, 20, n_shot, n_query)
    test_loss, test_acc = compute_loss_acc(Flag, model, support, query, labels, 
                                20, num_support, num_queries, emb_dim)
    
    test_losses.append(test_loss.numpy())
    test_acc.append(test_acc.numpy())
